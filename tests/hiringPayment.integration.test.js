...
test('hiring payment integrity integration', { skip: !(uri && dbName?.endsWith('_test')) && 'Set isolated Atlas test variables.' }, async (context) => {
  process.env.NODE_ENV = 'test'; process.env.MONGODB_URI = uri; process.env.MONGODB_DB_NAME = dbName; process.env.JWT_SECRET = randomBytes(48).toString('hex'); process.env.CLIENT_ORIGINS = 'http://localhost:5173'; process.env.STRIPE_SECRET_KEY = 'sk_test_phase8'; process.env.LAWYER_PUBLISHING_FEE_CENTS = '5000'
  ...
  await mongoose.connect(uri, { dbName }); const label = `pay-${randomBytes(5).toString('hex')}`; const emails = [`${label}-user@x.test`, `${label}-lawyer@x.test`]
  ...
  const [user, lawyer] = await User.create([{ fullName: 'Pay User', email: emails[0], role: 'user' }, { fullName: 'Pay Lawyer', email: emails[1], role: 'lawyer' }])
